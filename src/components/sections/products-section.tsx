...
export function ProductsSection() {
  ...
  function setVariant(i:number,k:keyof Variant,v:any){setForm((f:any)=>({...f,variants:(Array.isArray(f.variants)?f.variants:[]).map((x:Variant,n:number)=>n===i?{...x,[k]:v}:x)}))}
  ...
  function openEdit(p:Product){setEditing(p);setForm({name:p.name,description:p.description||'',categoryId:p.categoryId,brandId:p.brandId||'',gender:p.gender||'unisex',season:p.season||'all',material:p.material||'',image:p.image||'',variants:(Array.isArray(p.variants)?p.variants:[]).map((v:Variant)=>({...blankVariant(),...v,barcode:v.barcode||'',size:v.size||'',color:v.color||'',material:v.material||''}))});setVariantOpen(0);setOpen(true)}
  function addVariant(){setForm((f:any)=>({...f,variants:[...(Array.isArray(f.variants)?f.variants:[]),{...blankVariant()}] }));setVariantOpen((Array.isArray(form.variants)?form.variants:[]).length)}
