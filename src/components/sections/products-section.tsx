...
export function ProductsSection() {
  ...
  function openCreate(){setEditing(null);setForm({...blankProduct(),categoryId:categories[0]?.id||'',variants:[blankVariant()]});setVariantOpen(0);setShowExtraInfo(false);setShowCodes(false);setShowWholesale(false);setShowUnits(false);setOpen(true)}
  ...
  return <div className="space-y-4 pb-28">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-black">المنتجات</h2><p className="text-sm text-muted-foreground">{filtered.length} من {safeProducts.length} منتج</p></div><div className="flex gap-2"><Button variant="outline" className="h-14 rounded-2xl" onClick={()=>setSettingsOpen(true)}><Tag className="me-2"/> التصنيفات</Button><Button className="h-14 rounded-2xl px-5 font-black" onClick={openCreate}><Plus className="me-2"/> منتج جديد</Button></div></div>
