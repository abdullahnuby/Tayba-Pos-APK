...
export function SuppliersSection() {
  ...
  const { data: suppliersData, isLoading } = useQuery<Supplier[] | { items?: Supplier[] }>({
    queryKey: ['suppliers'],
    queryFn: async () => (await fetch('/api/suppliers')).json(),
  })

  const suppliers: Supplier[] = Array.isArray(suppliersData) ? suppliersData : (suppliersData?.items ?? [])
          ...
          <div className="max-h-[60dvh] overflow-y-auto px-5 py-4">
            {statementLoading ? (
              ...
              <div className="space-y-2">
                {(Array.isArray(statementData?.items) ? statementData.items : []).map((item) => (
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <button type="button" className="flex h-11 w-full items-center justify-center rounded-xl border bg-background font-black" onClick={() => openNumericPad({ value: String(payAmount), title: 'مبلغ دفعة المورد', min: 0.01, max: payDialog?.balance || 0, decimal: true, onCommit: v => setPayAmount(Number(v) || 0) })}>{payAmount || 0}</button>
            </div>
            <div className="space-y-1.5">
