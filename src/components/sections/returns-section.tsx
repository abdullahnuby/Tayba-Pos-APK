...
interface Sale { id: string; invoiceNo: string; date: string; total: number; status: string; paymentMethod?: RefundMethod; customer?: { name: string } | null; items: SaleItem[]; returns?: { items: SaleReturnItem[] }[] }
...
export function ReturnsSection() {
  ...
  const returns = Array.isArray(returnsData?.items) ? returnsData.items : []
  ...
  async function openNewReturn(sale: Sale) {
    ...
    try {
      ...
      const defaultRefund: RefundMethod =
        detail.paymentMethod === 'card' || detail.paymentMethod === 'cash'
          ? detail.paymentMethod
          : detail.paymentMethod === 'credit'
            ? 'credit'
            : 'cash'
      setRefundMethod(defaultRefund)
      setReturnItems((Array.isArray(detail.items) ? detail.items : []).map((it) => ({
        saleItemId: it.id,
  ...
  function submitReturn() {
    ...
    createReturn.mutate({
      ...
      refundMethod,
      idempotencyKey: `return-${selectedSale.id}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        ...
        <DialogContent className="flex max-h-[92dvh] flex-col gap-0 overflow-hidden p-0 sm:max-w-[820px]">
          ...
          {!selectedSale ? <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-6">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-6">
              ...
              <div className="mt-3 space-y-2 md:hidden">{(selectedSale.items || []).map((it) => { const remaining = remainingByItem.get(it.id) ?? Number(it.quantity || 0); const ret = returnItems.find((r) => r.saleItemId === it.id); const qty = ret?.quantity || 0; return <div key={it.id} className="rounded-xl border p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="font-medium">{it.variant.product.name}</div><div className="mt-1 text-xs text-muted-foreground">{it.variant.size || 'بدون مقاس'} · {it.variant.color || 'بدون لون'} · <span className="font-mono">{it.variant.sku}</span></div></div><div className="text-left text-sm font-semibold">{formatEGP(it.unitPrice)} ج.م</div></div><div className="mt-3 grid grid-cols-3 items-end gap-2"><div><div className="text-[11px] text-muted-foreground">مباعة</div><div className="font-semibold">{it.quantity}</div></div><div><div className="text-[11px] text-muted-foreground">متبقي للإرجاع</div><div className="font-semibold">{remaining}</div></div><div><Label className="text-[11px]">المرتجع</Label><button type="button" disabled={remaining === 0} onClick={() => openNumericPad({ value: String(qty), title: `كمية مرتجع ${it.variant.product.name}`, min: 0, max: remaining, onCommit: v => { const next = Math.min(remaining, Math.max(0, Math.floor(Number(v) || 0))); setReturnItems(items => items.map(r => r.saleItemId === it.id ? { ...r, quantity: next } : r)) } })} className="mt-1 flex h-11 w-full items-center justify-center rounded-xl border font-semibold disabled:opacity-40">{qty}</button></div></div></div> })}</div>
              <div className="mt-3 hidden md:block rounded-md border"><Table><TableHeader><TableRow><TableHead>المنتج</TableHead><TableHead>SKU</TableHead><TableHead className="text-center">مباعة</TableHead><TableHead className="text-center">متبقي</TableHead><TableHead className="text-center">المرتجع</TableHead><TableHead className="text-center">السعر</TableHead></TableRow></TableHeader><TableBody>{(selectedSale.items || []).map((it) => { const remaining = remainingByItem.get(it.id) ?? Number(it.quantity || 0); const ret = returnItems.find((r) => r.saleItemId === it.id); return <TableRow key={it.id}><TableCell className="text-xs">{it.variant.product.name} ({it.variant.size || '—'} / {it.variant.color || '—'})</TableCell><TableCell className="font-mono text-xs">{it.variant.sku}</TableCell><TableCell className="text-center">{it.quantity}</TableCell><TableCell className="text-center font-semibold">{remaining}</TableCell><TableCell><button type="button" disabled={remaining === 0} onClick={() => openNumericPad({ value: String(ret?.quantity || 0), title: `كمية مرتجع ${it.variant.product.name}`, min: 0, max: remaining, onCommit: v => { const next = Math.min(remaining, Math.max(0, Math.floor(Number(v) || 0))); setReturnItems(items => items.map(r => r.saleItemId === it.id ? { ...r, quantity: next } : r)) } })} className="mx-auto flex h-10 w-20 items-center justify-center rounded-xl border text-center font-semibold disabled:opacity-40">{ret?.quantity || 0}</button></TableCell><TableCell className="text-center">{formatEGP(it.unitPrice)}</TableCell></TableRow> })}</TableBody></Table></div>
