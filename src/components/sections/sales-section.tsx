...
export function SalesSection({ user }: { user: SessionUser }) {
  ...
  const openShift = shiftData?.items?.find(
    x => x.status === 'open'
  )
  ...
  const products = (
    Array.isArray(productsQuery.data?.items)
  ...
  function hasPackPricing(v: Variant) {
    return (
      (v.quarterDozenPrice ?? 0) > 0 ||
      (v.halfDozenPrice ?? 0) > 0 ||
      (v.dozenPrice ?? 0) > 0
    )
  }
  ...
  function addVariant(
    ...
    if (v.quantity < factor) {
      return toast.error(
        'لا يوجد مخزون كافٍ لهذه الوحدة'
      )
    }
    ...
    if (
      ...
      return toast.error(
        `المبلغ المدفوع (${safePaid.toFixed(
          2
        )}) أقل من الإجمالي (${safeTotal.toFixed(
          2
        )})`
      )
    }
    ...
    return (
      <Card className="mx-auto mt-8 max-w-xl p-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          لا يمكن للكاشير إصدار فواتير قبل فتح الوردية.
        </p>
    ...
    >
      ...
      <div className="pos-topbar shrink-0 border-b bg-background px-3 py-2 sm:px-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {openShift && (
              <Badge className="hidden xs:inline-flex">
                وردية مفتوحة
              </Badge>
            )}
          </div>
        ...
        {historical && (
          <div className="mt-2.5 flex flex-wrap items-end gap-3 rounded-2xl border bg-muted/30 p-3">
            ...
            <p className="text-xs text-muted-foreground">
              لإدخال فواتير الورق بتاريخها الحقيقي.
            </p>
          </div>
        )}
      </div>
                ...
                .map(
                  (v: Variant) => (
                    ...
                    >
                      ...
                      <div className="mt-3 text-lg font-black text-primary">
                        {money(
                          v.sellPrice
                        )}
                      </div>
      ...
      >
        <DialogContent className="w-[calc(100vw-1rem)] max-w-md rounded-3xl p-4">
          ...
          <Button
            className="mt-2 h-14 w-full rounded-2xl text-base font-black"
            disabled={
              saveSale.isPending
            }
            onClick={submit}
