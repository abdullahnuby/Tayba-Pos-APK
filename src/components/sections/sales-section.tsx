...
import { Banknote, Barcode, CheckCircle2, Eye, History, Menu, MessageCircle, Pause, Play, Plus, Printer, ReceiptText, Search, Share2, Trash2, UserPlus, X } from 'lucide-react'
...
interface Sale { id: string; invoiceNo: string; date: string; total: number; paid: number; change: number; paymentMethod: string; status: string; customer?: { name: string; phone?: string | null } | null; items: Array<{ id: string; quantity: number; total: number; variant: { product: { name: string }; sku: string; size: string | null; color: string | null } }> }
...
export function SalesSection({ user }: { user: SessionUser }) {
  ...
  const products = (Array.isArray(productsQuery.data?.items) ? productsQuery.data.items : []).map(p => ({ ...p, variants: Array.isArray(p.variants) ? p.variants : [] }))
  const customers: Customer[] = Array.isArray(customersQuery.data) ? customersQuery.data : (Array.isArray((customersQuery.data as any)?.items) ? (customersQuery.data as any).items as Customer[] : [])
  const sales = (Array.isArray(salesQuery.data?.items) ? salesQuery.data.items : []).map(s => ({ ...s, items: Array.isArray(s.items) ? s.items : [] }))
  ...
  const visible = useMemo(() => {
    ...
    return products.filter(p => (category === 'all' || (p.category?.id || 'none') === category) && (!q || p.name.toLowerCase().includes(q) || p.variants.some(v => v.sku.toLowerCase().includes(q) || (v.barcode || '').includes(q)))).slice(0, 200)
  ...
  useEffect(() => { setProductPage(0) }, [search, category, visible.length])
  useEffect(() => { if (productPage >= productPageCount) setProductPage(Math.max(0, productPageCount - 1)) }, [productPage, productPageCount])
  ...
  const saveCustomer = useMutation<Customer, Error, { name: string; phone: string }>({
    ...
    onError: (e: Error, variables: { name: string; phone: string }) => {
      if (e.message.includes('يحتاج موافقة المدير') || e.message.includes('خارج حدود الكاشير')) {
        setPendingSalePayload(variables as Record<string, unknown>)
  ...
  const saveSale = useMutation({
    ...
    onSuccess: (sale: Sale) => { qc.invalidateQueries({ queryKey: ['sales'] }); qc.invalidateQueries({ queryKey: ['pos-products'] }); qc.invalidateQueries({ queryKey: ['register-sessions'] }); setCheckout(false); if (sale.status === 'draft') { resetSale(); toast.success(`تم تعليق الفاتورة ${sale.invoiceNo}`) } else { setPrinting(sale); resetSale(); toast.success(`تمت الفاتورة ${sale.invoiceNo}`) } },
  ...
  function holdSale() {
    ...
    saveSale.mutate({ customerId: customerId || undefined, discount, paid: 0, paymentMethod: paymentMethod === 'credit' ? 'credit' : 'cash', status: 'draft', items: cart.map(i => ({ variantId: i.variantId, quantity: i.quantity * i.factor, unitPrice: i.price / i.factor })), notes: 'فاتورة معلقة', idempotencyKey: `${user.id}-draft-${Date.now()}-${Math.random().toString(36).slice(2,8)}` } as any)
  }
  async function resumeDraft(s: Sale) {
    try { const r = await fetch(`/api/sales/${s.id}`); const j = await r.json(); if (!r.ok) throw new Error(j.error || 'تعذر تحميل الفاتورة'); const next = (j.items || []).map((item:any) => { const v = products.flatMap(p=>p.variants).find(v=>v.id===item.variant_id || v.id===item.variantId); return { variantId: item.variant_id || item.variantId, name: item.product_name || item.variant?.product?.name || v?.product?.name || 'صنف', sku: item.sku || v?.sku || '', size: item.size ?? v?.size ?? null, color: item.color ?? v?.color ?? null, price: Number(item.unit_price ?? item.unitPrice ?? v?.sellPrice ?? 0), quantity: Number(item.quantity||0), max: Number(v?.quantity ?? 0) + Number(item.quantity||0), unit: 'piece', factor: 1 } as CartItem }); setCart(next); setCustomerId(j.customer_id || ''); setDiscount(Number(j.discount||0)); setPaymentMethod('cash'); setPaid(0); setViewing(null); toast.success(`تم استئناف الفاتورة ${s.invoiceNo}`) } catch(e:any){ toast.error(e?.message || 'تعذر الاستئناف') }
  }
  ...
  function approveAndRetry() {
    ...
    if (!managerUsername.trim() || !/^\d{4}$/.test(managerPin)) { toast.error('أدخل اسم المدير وPIN من 4 أرقام'); return }
    saveSale.mutate({ ...pendingSalePayload, managerApproved: true, managerUsername: managerUsername.trim(), managerPin })
    setManagerDialog(false); setManagerUsername(''); setManagerPin(''); setPendingSalePayload(null)
  }

  function roundMoney(value: number) { return Math.round((Number(value) || 0) * 100 + Number.EPSILON) / 100 }
  function moneyCents(value: number) { return Math.round((Number(value) || 0) * 100) }
  function submit() {
    if (!cart.length) return toast.error('أضف صنفًا أولًا')
  ...
  function receiptText(s: Sale) {
    ...
    for (const item of s.items || []) lines.push(`${item.variant?.product?.name || 'صنف'} × ${item.quantity} = ${money(item.total)}`)
  ...
  return <div className={(user.role === 'cashier' ? 'cashier-pos ' : '') + 'flex h-[100dvh] flex-col overflow-hidden bg-muted/10 lg:h-auto lg:min-h-[calc(100vh-8rem)] lg:rounded-3xl lg:border'}>
    {/* Top bar */}
    <div className="pos-topbar shrink-0 border-b bg-background px-3 py-2 sm:px-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" className="size-10 rounded-2xl" disabled={!cart.length || saveSale.isPending} onClick={holdSale} aria-label="تعليق الفاتورة"><Pause className="size-5" /></Button>
    ...
    {/* Categories — cashier gets a compact no-scroll selector */}
    <div className="pos-categories shrink-0 border-b bg-background px-3 py-1.5 sm:px-3">
      {user.role === 'cashier' ? (
        <select aria-label="تصنيف المنتجات" value={category} onChange={e => setCategory(e.target.value)} className="h-9 w-full rounded-xl border bg-card px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/30">
    ...
    <div className="pos-body flex min-h-0 flex-1 flex-col overflow-hidden lg:grid lg:grid-cols-[1fr_400px]">
      <div className="pos-products-pane min-h-0 flex-1 overflow-y-auto p-3 sm:p-3">
        {productsQuery.isLoading ? <div className="grid grid-cols-3 gap-2">{Array.from({ length: 9 }).map((_, i) => <Skeleton key={i} className="h-[6.5rem] rounded-2xl" />)}</div> :
          ...
          <div className="pos-product-grid grid grid-cols-3 gap-2 sm:gap-2">
            {visiblePage.map(p => {
          ...
          {productPageCount > 1 && <div className="mt-2 flex items-center justify-center gap-2">
            <Button type="button" variant="outline" size="sm" className="h-9 rounded-xl px-3" disabled={productPage === 0} onClick={() => setProductPage(p => Math.max(0, p - 1))}>السابق</Button>
            <span className="text-[11px] font-bold text-muted-foreground">{productPage + 1} / {productPageCount}</span>
            <Button type="button" variant="outline" size="sm" className="h-9 rounded-xl px-3" disabled={productPage >= productPageCount - 1} onClick={() => setProductPage(p => Math.min(productPageCount - 1, p + 1))}>التالي</Button>
          </div>}
      </div>
      ...
      <div className="pos-cart min-h-0 flex max-h-[42dvh] shrink-0 flex-col border-t bg-background lg:max-h-none lg:h-full lg:border-t-0 lg:border-r">
        ...
        {customerPickerOpen && <div className="pos-customer-picker absolute end-2 top-12 z-30 flex max-w-[calc(100%-1rem)] gap-2 overflow-hidden rounded-2xl border bg-background p-2 shadow-xl">
          ...
          {customers.map((c: Customer) => <button key={c.id} type="button" onClick={() => { setCustomerId(c.id); setCustomerPickerOpen(false) }} className={`min-w-max rounded-xl border px-3 py-2 text-xs font-bold ${customerId === c.id ? 'border-primary bg-primary/10' : 'bg-card'}`}>{c.name}</button>)}
        ...
        <div className="shrink-0 border-t bg-card p-2.5 pb-[max(.65rem,env(safe-area-inset-bottom))]">
          <div className="flex items-stretch gap-2">
            <div className="flex shrink-0 flex-col items-center justify-center rounded-2xl border px-2 text-[11px]"><span className="text-muted-foreground">الخصم</span><button type="button" className="h-9 w-16 rounded-lg px-1 text-center font-black active:scale-95" onClick={() => openNumericPad({ value: String(discount), title: 'قيمة الخصم', min: 0, max: subtotal, decimal: true, onCommit: v => setDiscount(Math.max(0, Math.min(subtotal, Number(v) || 0))) })} aria-label="قيمة الخصم">{discount}</button></div>
    ...
    {/* Variant picker */}
    <Dialog open={!!selectedProduct} onOpenChange={(o: boolean) => !o && setSelectedProduct(null)}><DialogContent className="w-[calc(100vw-1rem)] max-w-xl rounded-3xl p-4"><DialogHeader><DialogTitle>اختيار المقاس واللون</DialogTitle><DialogDescription>{selectedProduct?.name}</DialogDescription></DialogHeader>{selectedProduct && <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{(selectedProduct.variants || []).filter((v: Variant) => v.quantity > 0).map((v: Variant) => <button key={v.id} type="button" onClick={() => handlePickVariant(v, selectedProduct.name)} className="min-h-28 rounded-3xl border p-4 text-right active:scale-[.98]"><div className="font-black">{v.size || 'مقاس عام'}</div><div className="mt-1 text-sm text-muted-foreground">{v.color || 'لون عام'}</div><div className="mt-3 text-lg font-black text-primary">{money(v.sellPrice)}</div><div className="mt-1 text-xs text-muted-foreground">متوفر {v.quantity}</div></button>)}</div>}</DialogContent></Dialog>
    ...
    <Dialog open={checkout} onOpenChange={v => !saveSale.isPending && setCheckout(v)}><DialogContent className="w-[calc(100vw-1rem)] max-w-md rounded-3xl p-4"><DialogHeader><DialogTitle>تأكيد البيع — {money(total)}</DialogTitle></DialogHeader>
      ...
      {paymentMethod !== 'credit' && <div className="mt-2"><Label>المبلغ المستلم</Label><button type="button" className="mt-1 flex h-12 w-full items-center justify-center rounded-xl border bg-background text-xl font-black tabular-nums" onClick={() => openNumericPad({ value: String(paid), title: 'المبلغ المستلم', min: 0, decimal: true, onCommit: v => setPaid(Math.max(0, Number(v) || 0)) })}>{paid}</button></div>}
    ...
    <Dialog open={managerDialog} onOpenChange={v => !saveSale.isPending && setManagerDialog(v)}><DialogContent className="rounded-3xl"><DialogHeader><DialogTitle>موافقة المدير مطلوبة</DialogTitle><DialogDescription>السعر خارج حدود الكاشير. استخدم اسم المدير وPIN من 4 أرقام للموافقة على الفاتورة.</DialogDescription></DialogHeader><div className="space-y-3 py-2"><div><Label>اسم مستخدم المدير</Label><Input value={managerUsername} onChange={e => setManagerUsername(e.target.value)} dir="ltr" autoFocus /></div><div><Label>PIN المدير</Label><button type="button" onClick={() => openNumericPad({ value: managerPin, title: 'PIN المدير — 4 أرقام', decimal: false, maxLength: 4, password: true, onCommit: setManagerPin })} className="flex h-14 w-full items-center justify-center rounded-2xl border bg-background text-xl font-black tracking-[0.55em]">{managerPin ? '•'.repeat(managerPin.length) : 'أدخل PIN من 4 أرقام'}</button></div></div><DialogFooter><Button variant="outline" onClick={() => setManagerDialog(false)}>إلغاء</Button><Button onClick={approveAndRetry} disabled={saveSale.isPending || !managerUsername.trim() || managerPin.length!==4}>تأكيد الموافقة</Button></DialogFooter></DialogContent></Dialog>
    <Dialog open={!!printing} onOpenChange={v => !v && setPrinting(null)}><DialogContent className="rounded-3xl"><DialogHeader><DialogTitle>الفاتورة تمت بنجاح</DialogTitle></DialogHeader>{printing && <div id="printable-invoice" className="rounded-xl border bg-white p-4 text-black"><div className="text-center text-xl font-black">طيبة</div><div className="mt-2 text-sm">فاتورة: {printing.invoiceNo}</div><div className="text-sm">التاريخ: {formatDateTime(printing.date)}</div>{(printing.items || []).map(i => <div key={i.id} className="flex justify-between border-b py-2 text-sm"><span>{i.variant?.product?.name || 'صنف'} × {i.quantity}</span><b>{money(i.total)}</b></div>)}<div className="mt-3 flex justify-between font-black"><span>الإجمالي</span><span>{money(printing.total)}</span></div></div>}<div className="grid grid-cols-2 gap-2"><Button onClick={() => window.print()}><Printer className="size-4" /> طباعة</Button><Button variant="outline" onClick={() => printing && shareReceipt(printing)}><Share2 className="size-4" /> مشاركة</Button></div><div className="grid grid-cols-2 gap-2"><Button variant="outline" className="h-12 rounded-2xl" onClick={() => printing && sendReceiptWhatsApp(printing)}><MessageCircle className="size-4" /> إرسال واتساب</Button><Button className="h-12 rounded-2xl" onClick={() => setPrinting(null)}><CheckCircle2 className="size-4" /> فاتورة جديدة</Button></div></DialogContent></Dialog>

    {/* History */}
    <Dialog open={historyOpen} onOpenChange={setHistoryOpen}><DialogContent className="max-h-[90dvh] overflow-y-auto rounded-3xl sm:max-w-2xl"><DialogHeader><DialogTitle>سجل الفواتير</DialogTitle></DialogHeader>{salesQuery.isLoading ? <Skeleton className="h-24" /> : <div className="space-y-2">{sales.map(s => <div key={s.id} className="rounded-xl border p-3"><div className="flex justify-between"><b>{s.invoiceNo}</b><Badge variant={saleStatusBadgeVariant(s.status)}>{saleStatusLabel(s.status)}</Badge></div><div className="mt-1 text-xs text-muted-foreground">{formatDateTime(s.date)} · {s.customer?.name || 'عميل نقدي'}</div><div className="mt-2 flex justify-between gap-2"><b>{money(s.total)}</b><div className="flex gap-1"><Button size="sm" variant="outline" onClick={() => setViewing(s)}><Eye className="size-4" /> عرض</Button>{s.status==='draft'&&<Button size="sm" onClick={() => resumeDraft(s)}><Play className="size-4" /> استئناف</Button>}</div></div></div>)}</div>}</DialogContent></Dialog>
