...
export function UsersSection() {
  ...
  const { data: usersData, isLoading } = useQuery<
    AppUser[] | { items?: AppUser[] }
  >({
    queryKey: ['users'],
    queryFn: async () => (await fetch('/api/users')).json(),
  })

  const data: AppUser[] = Array.isArray(usersData)
    ? usersData
    : Array.isArray((usersData as any)?.items)
      ? (usersData as any).items as AppUser[]
      : []
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        'تم إنشاء المستخدم — الدخول باسم المستخدم وPIN من 4 أرقام'
      )
  ...
  const patchMutation = useMutation({
    mutationFn: async ({
      ...
      if (!res.ok) {
        throw new Error(body.error || 'فشل التعديل')
      }
    ...
    <div className="space-y-6">
        ...
        >
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                يسجل الدخول باسم المستخدم وPIN من 4 أرقام فقط
              </DialogDescription>
            </DialogHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            البيانات محفوظة محليًا على الجهاز وتتم مزامنتها لاحقًا
          </CardDescription>
        </CardHeader>
                        ...
                        >
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogTitle>
                                تعديل PIN الدخول — {u.name}
                              </DialogTitle>

                              <DialogDescription>
                                ده نفس PIN الدخول للمستخدم،
                                ويُستخدم أيضًا لتأكيد فتح وإغلاق الوردية.
                              </DialogDescription>
                            ...
                            <div className="space-y-2 py-2">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  openNumericPad({
                                    value: pinValue,
                                    title: 'PIN الجديد — 4 أرقام',
                                    decimal: false,
                                    maxLength: 4,
                                    password: true,
                                    onCommit: setPinValue,
                                  })
                                }
                              >
                                {pinValue
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                الإجراء ده نهائي.
                                لو عايز توقف دخوله بس،
                                استخدم زر التعطيل بدل الحذف.
                              </AlertDialogDescription>
