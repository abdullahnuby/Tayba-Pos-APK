...
export async function dashboard(from?:string|null,to?:string|null){
 ...
 const lastDay=r.dailyTrend.find((x:any)=>x.date===end) || r.dailyTrend.at(-1) || {sales:0,profit:0,purchases:0,returns:0}
 const todayExpenseRows=query<any>(db,"SELECT COALESCE(SUM(amount),0) total FROM expenses WHERE date(date)=date(?)",[end])[0]?.total||0
 const todayGross=Number(lastDay.profit||0)
 const todayNet=todayGross-Number(todayExpenseRows||0)
 return {from:start,to:end,todaySales:Number(todayRows.todaySales||0),todayGrossProfit:todayGross,todayNetProfit:todayNet,todayProfit:todayNet,todayExpenses:Number(todayExpenseRows||0),todaySalesCount:Number(todayRows.todayInvoices||0),lowStockCount:Number(low),outOfStockCount:Number(out),inventoryValue:Number(r.inventory.cost_value||0),retailValue:Number(r.inventory.retail_value||0),potentialProfit:Number(r.inventory.potential_profit||0),customerBalance:Number(r.customerBalance||0),supplierBalance:Number(r.supplierBalance||0),periodNetSales:Number(r.netSales||0),periodGrossProfit:Number(r.grossProfit||0),periodNetProfit:Number(r.netProfit||0),periodExpenses:Number(r.totalExpenses||0),salesTrend:r.dailyTrend.map((x:any)=>({date:x.date,label:x.date.slice(5),sales:Number(x.sales||0),profit:Number(x.profit||0)})),topProducts:r.bestSelling.slice(0,5),recentSales:query<any>(db,"SELECT s.*,s.date date,s.created_at createdAt,COALESCE(c.name,'عميل نقدي') customerName,(SELECT COUNT(*) FROM sale_items si WHERE si.sale_id=s.id) itemsCount FROM sales s LEFT JOIN customers c ON c.id=s.customer_id WHERE s.status='completed' ORDER BY s.date DESC LIMIT 8").map((x:any)=>({...x,date:x.date||x.createdAt||null})),lowStockList:lowRows,reorderList:lowRows.map(x=>({...x,suggestedOrder:Math.max(Number(x.reorder_qty||0)-Number(x.quantity||0),0)})),todayByMethod}
}
