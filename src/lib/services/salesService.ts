...
export async function completeSale(input:CompleteSaleInput):Promise<CompletedSale>{
 ...
 return withTransaction(db=>{
  ...
  run(db,`INSERT INTO sales(id,invoice_no,user_id,customer_id,register_session_id,date,subtotal,discount,total,paid,change,payment_method,status,notes,idempotency_key,created_role,manager_approved) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[saleId,invoiceNo,input.userId,input.customerId??null,input.registerSessionId,input.date??new Date().toISOString(),subtotal,discount,total,effectivePaid,change,input.paymentMethod,status,notes,input.idempotencyKey??null,input.role??'cashier',input.managerApproved?1:0])
...
export async function resumeSale(input:{userId:string;saleId:string;managerApproved?:boolean}) {
 return withTransaction(db=>{
  ...
  const items=query<any>(db,'SELECT * FROM sale_items WHERE sale_id=?',[input.saleId])
  if(!items.length) throw new Error('لا يمكن استئناف مسودة بدون بنود')
  const role=(sale.created_role||'cashier') as 'admin'|'manager'|'cashier'
  const managerApproved=Number(sale.manager_approved||0)===1 || !!input.managerApproved
  if(input.managerApproved && Number(sale.manager_approved||0)!==1) run(db,"UPDATE sales SET manager_approved=1 WHERE id=?",[input.saleId])
  if(role==='cashier' && !managerApproved && Number(sale.discount||0)>Number(sale.subtotal||0)*0.05) throw new Error('لا يمكن استئناف الفاتورة: خصم الكاشير يتجاوز 5% ويحتاج موافقة المدير')
  for(const item of items){
   const priceCheck=checkLocalSalePrice(db,item.variant_id,Number(item.unit_price),role,managerApproved)
   if(!priceCheck.ok) throw new Error(`لا يمكن استئناف الفاتورة: ${priceCheck.error||'السعر غير مسموح'}`)
   const stock=query<{quantity:number}>(db,'SELECT quantity FROM product_variants WHERE id=?',[item.variant_id])[0]
   if(!stock||Number(stock.quantity)<Number(item.quantity))throw new Error('المخزون الحالي غير كافٍ لاستئناف الفاتورة')
  }
