...
export async function returnSale(input:{userId:string;saleId:string;customerId?:string|null;lines:SaleReturnLine[];refundMethod?:PaymentMethod;reason?:string;notes?:string;idempotencyKey?:string}){
 return withTransaction(db=>{
  ...
  run(db,`INSERT INTO sale_returns(id,return_no,sale_id,customer_id,subtotal,total,reason,notes,status,refund_method,idempotency_key) VALUES(?,?,?,?,?,?,?,?, 'completed',?,?)`,[id,no,input.saleId,customerId,total,total,input.reason??null,noteText,refund,input.idempotencyKey??null])
