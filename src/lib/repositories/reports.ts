...
export async function fullReports(from?:string|null,to?:string|null){
  ...
  const operatingNetProfit=grossProfit-totalExpenses
  const totalUnits=saleItems.reduce((a,r)=>a+Number(r.quantity||0),0)
  return {
    from:start,to:end,
    salesCount:sales.length,purchasesCount:purchases.length,returnsCount:saleReturns.length,totalUnits,
    totalSales:+totalSales.toFixed(2),netSales:+netSales.toFixed(2),totalReturns:+totalReturns.toFixed(2),totalPurchases:+totalPurchases.toFixed(2),
    cogs:+(cogs-returnCogs).toFixed(2),grossProfit:+grossProfit.toFixed(2),totalExpenses:+totalExpenses.toFixed(2),netProfit:+operatingNetProfit.toFixed(2),
    totalProfit:+operatingNetProfit.toFixed(2),profitMargin:netSales>0?+(operatingNetProfit/netSales*100).toFixed(2):0,grossMargin:netSales>0?+(grossProfit/netSales*100).toFixed(2):0,
    ...
    bestSelling:[...bestMap.values()].sort((a,b)=>b.qty-a.qty).slice(0,10),
    salesByCategory:[...catMap].map(([name,value])=>({name,value:+Number(value).toFixed(2)})).sort((a,b)=>b.value-a.value),
    salesByMethod:[...methodMap].map(([name,value])=>({name,value:+Number(value).toFixed(2)})),
    dailyTrend:daily,
    inventory:{...inventory,cost_value:Number(inventory.cost_value||0),retail_value:Number(inventory.retail_value||0),potential_profit:Number(inventory.retail_value||0)-Number(inventory.cost_value||0)},
    customerBalance:Number(customerBalance),supplierBalance:Number(supplierBalance)
  }
}
